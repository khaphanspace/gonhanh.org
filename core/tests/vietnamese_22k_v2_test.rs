//! Test Vietnamese 22k word list for V2 Engine.
//! Converts Vietnamese words to Telex input and verifies engine output.

use gonhanh_core::v2::{Action, Engine};
use std::fs::File;
use std::io::Write;
...
/// Type word helper for V2 Engine
fn type_word(e: &mut Engine, input: &str) -> String {
    ...
    for c in input.chars() {
        ...
        let r = e.on_key_ext(key, is_caps, false, false);
        if r.action == Action::Send as u8 {
            for _ in 0..r.backspace {
                screen.pop();
            }
...
fn vietnamese_22k_v2_coverage() {
    ...
    println!("\n=== Vietnamese 22k V2 Test Results ===");
    ...
    if let Ok(mut f) = File::create("tests/data/vietnamese_22k_v2_failures.txt") {
        for (word, telex, expected, actual) in &failures {
            let _ = writeln!(f, "{}\t{}\t{}\t{}", word, telex, expected, actual);
        }
        println!("\nFailures written to tests/data/vietnamese_22k_v2_failures.txt");
    }
    ...
    assert!(
        ...
        "Vietnamese 22k V2 single syllable pass rate {:.2}% is below threshold {:.1}%",
        pass_rate,
        MIN_PASS_RATE
    );
}
...
fn vietnamese_22k_v2_compound() {
    ...
    println!("\n=== Vietnamese 22k V2 Compound Words Test ===");
    ...
    assert!(
        pass_rate >= MIN_PASS_RATE,
        "Vietnamese 22k V2 compound words pass rate {:.2}% is below threshold {:.1}%",
        pass_rate,
        MIN_PASS_RATE
    );
}
