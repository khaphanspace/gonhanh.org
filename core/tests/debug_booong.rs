...
#[test]
fn debug_booong() {
    // Test "booong" - b+o+o+o+n+g → triple 'o' should collapse to double "boong"
    let keys_seq = [keys::B, keys::O, keys::O, keys::O, keys::N, keys::G];
    ...
    eprintln!("buffer char count = {}", buf.chars().count());
    eprintln!("raw char count = {}", raw.chars().count());
