//! English 100k Words Test for V2 Engine
//!
...
fn english_100k_v2_failures() {
    ...
    // Load English dictionary for dictionary-based restore
    engine.load_english_dict(&content);
    ...
    if !failures.is_empty() {
        ...
        let failure_list: Vec<String> = failures
            ...
            .collect();
        fs::write(
            "tests/data/english_100k_v2_failures.txt",
            failure_list.join("\n"),
        )
        .expect("Failed to write failures file");
        println!("\nFull list written to: tests/data/english_100k_v2_failures.txt");
    }
    ...
    assert!(
        ...
        "English 100k V2 pass rate {:.2}% is below threshold {:.1}%",
        pass_rate,
        MIN_PASS_RATE
    );
}
