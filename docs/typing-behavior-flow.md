...
> **Note:** "vả" is valid Vietnamese (slap/fig). No restore triggered - VN interpretation wins.
...
> **Note:** Double `rr` reverts hỏi mark → enters EN mode. Buffer gets raw 'r' added.
...
> - Steps 7+: VN=~ means "invalid word but not impossible pattern"
> - No restore because repeated consonants ≠ foreign cluster
...
> **Note:** 'oo' forms circumflex (ô) non-consecutively, like 'ee' → ê. Result is valid VN word "tốt" (good).
...
> **Note:** 'l' can't be final consonant in Vietnamese → restore triggered at step 6. 's' restored as raw character.
...
> **Note:** Double 'ss' reverts sắc → enters EN mode. Buffer gets raw 's' added.
...
| VN | ✓=valid, ✗=impossible (triggers restore), ~=invalid word but possible pattern |
...
| Impossible consonant cluster (`xt`, `xp`) | ✗ | **Triggered** immediately (once) |
| Foreign cluster early (`cl`) | ✗ | **Foreign** (stay raw, no transform) |
...
| `d i s s t` + space | 6 | `dist ` | - (double 'ss' revert) |
